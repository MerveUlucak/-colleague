...
export default function app() {
  ...
  return (
    <div>
      ...
      <div
        onClick={() =>
          setFlashCard((prev) => ({ ...prev, status: !prev.status }))
        }
        className={`flash-card ${flashCard.status ? 'flipped' : ''}`}
      >
        <div className="flash-card-inner">
          <div className="flash-card-front">
            <p className="question">{flashCard && flashCard.question}</p>
            <ol type="a">
              {flashCard &&
          ...
          <div className="flash-card-back">
            <p className="answer">{flashCard && flashCard.answer}</p>
            <p>{flashCard && flashCard.explanation}</p>
          </div>
        </div>
      </div>
    </div>
